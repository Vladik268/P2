	...
	Hooks:Add("NetworkReceivedData", class_name.."2", function(peer_id, id, data)
		if id == class_name then
			...
			if c.notify_on_join and sender and not c.recived_peers[sender:user_id()] then
				...
				managers.chat:_receive_message(1, "Advance Anti Crash", string.format("%s also have Advance Anti Crash installed.", sender:name()), (c.notify_color or tweak_data.system_chat_color))
			end
		end
	end)
end
