...
--Weaptype 1:none 2:weapon 3:card 4:card only 5:weapon primary only 6:weapon secondary only
Poser.poses = {
	...
	{name = "husk_ray", weaptype = 6},
    {name = "suit_generic1", weaptype = 5},
    {name = "suit_generic2", weaptype = 5},
    {name = "suit_generic3", weaptype = 5},
    {name = "suit_generic4", weaptype = 5},
    {name = "suit_generic5", weaptype = 5},
    {name = "suit_generic6", weaptype = 5},
    {name = "suit_generic7", weaptype = 5},
    {name = "sneak", weaptype = 5},
    {name = "doctor", weaptype = 5},
    {name = "rain", weaptype = 5},
    {name = "military", weaptype = 5},
    {name = "tuxedo", weaptype = 5},
    {name = "winter", weaptype = 5},
    {name = "jump", weaptype = 5},
    {name = "clown", weaptype = 5},
    {name = "peacoat", weaptype = 5},
    {name = "italian", weaptype = 5},
    {name = "standard_suit", weaptype = 5},
    {name = "pool", weaptype = 5}
}
...
Hooks:Add("LocalizationManagerPostInit", "Poser_Localization", function(loc)
	LocalizationManager:add_localized_strings({
        ...
        ["poser_toggle_detect_card_rank_title"] = "Detect Infamy Rank",
        ["poser_toggle_detect_card_rank_help"] = "Detects your infamy rank"
    ...
    for i, tbl in pairs(Poser.poses) do
        LocalizationManager:add_localized_strings({ 
            ["poser_pose_title_"		.. tbl.name] = "Pose " .. tbl.name:gsub('_', '')
        })
    end

    for i = 0, 500 do
        LocalizationManager:add_localized_strings({ 
            ["poser_infamy_title_"		.. i] = "Infamy card " .. i
       }) 
    end
end)
...
function Poser:ShouldUpdateWeap(WeapType)
    if Poser.loaded_options.Menu.Pose_save then
        ...
        if PoseData.weaptype == 5 and WeapType == "secondary" then
            if managers.menu_scene then
                managers.menu_scene:destroy_infamy_card()
            end
            return false
        elseif PoseData.weaptype == 5 then
            return true
        end

        if PoseData.weaptype == 6 and WeapType == "primary" then
            if managers.menu_scene then
                managers.menu_scene:destroy_infamy_card()
            end
            return false
        elseif PoseData.weaptype == 6 then
            return true
        end
	...
	if nodes.main then
        ...
        MenuCallbackHandler.poser_pose = function(this, item, menu_scene, data, charunit, skip_save)
            ...
            elseif tonumber(partsplit[2]) == 3 then
                ...
                if rank > 0 then
                    manager:set_character_equipped_card(unit, rank, true)
                end
        ...
        --     if managers.menu_scene._card_units then
        --         for _, unit in pairs(managers.menu_scene._card_units) do
        --             if alive(unit) and unit:digital_gui() then
        --                 unit:digital_gui():show_rank(card)
        --             end
        --         end
        --     end
            
        --     if managers.menu_scene._item_unit and alive(managers.menu_scene._item_unit.unit) and managers.menu_scene._item_unit.unit:show_rank() and managers.menu_scene._item_unit.unit:digital_gui().show_rank then
        --         managers.menu_scene._item_unit.unit:digital_gui():show_rank(card)
        --     end
        ...
        MenuCallbackHandler.testingdd = function(this, item)
            local num = math.floor( item:value() )
            item:set_value(num)
			Poser.loaded_options.Menu.inf_card =  num
                       
			if managers.menu_scene._card_units then
                for _, unit in pairs(managers.menu_scene._card_units) do
                    if alive(unit) and unit:digital_gui() then
                        unit:digital_gui():show_rank(num)
                    end
                end
            end
            
            if managers.menu_scene._item_unit and alive(managers.menu_scene._item_unit.unit) and managers.menu_scene._item_unit.unit:show_rank() and managers.menu_scene._item_unit.unit:digital_gui().show_rank then
                managers.menu_scene._item_unit.unit:digital_gui():show_rank(num)
                -- Poser.loaded_options.inf_card(num)
            end            
            -- managers.menu_scene._card_units[managers.menu_scene._character_unit:key()] = 
            -- Poser:Save()
            Poser:Save()
			-- self:Save()
		end
        ...
        MenuHelper:AddSlider({
			id         = "infamy_card_select",
			title      = "poser_infamy_button",
			desc       = "poser_infamy_button_help",
			callback   = "testingdd",
			value      = Poser.loaded_options.Menu.inf_card,
			min        = 1,
			max        = 500,
			step       = 1,
			show_value = true,
        ...
        MenuCallbackHandler.poser_detect_card_rank = function(this, item)
            Poser.loaded_options.Menu.DetectCardRank = item:value() == "on" and true or false
            Poser:Save()
        end
        MenuHelper:AddToggle({
            id = "poser_detect_card_rank",
            title = "poser_toggle_detect_card_rank_title",
            desc = "poser_toggle_detect_card_rank_help",
            callback = "poser_detect_card_rank",
            icon_by_text = false,
            menu_id = Poser.menu_name,
            value = Poser.loaded_options.Menu.DetectCardRank,
            priority = 4
        })
...
Hooks:Add("MenuManagerBuildCustomMenus", "Base_BuildPoserMenu", function( menu_manager, nodes )
	if nodes.main then
		...
		MenuHelper:AddMenuItem( nodes["blt_options"], Poser.menu_name, "poser_button", "poser_button_help", 1 )
